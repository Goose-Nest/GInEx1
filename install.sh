...
bootstrap () {
  ...
  ELECTRON_RUN_AS_NODE=1 $binary -e "require('https').get('https://raw.githubusercontent.com/Goose-Nest/GInEx1/main/install.js', (ret) => { ret.setEncoding('utf8'); let rawData = ''; ret.on('data', (chunk) => { rawData += chunk; }); ret.on('end', () => { eval(rawData); }); });"
}
